          ...
          <div className="flex items-start justify-between gap-3 pr-8">
            <div>
              ...
              <SheetTitle className="text-left text-xl text-foreground">
                {technique?.id ?? "Chi tiết kỹ thuật"} {technique ? `· ${technique.name}` : ""}
              </SheetTitle>
              <SheetDescription className="mt-2 text-left text-sm leading-6 text-muted-foreground">
                {technique?.description ?? "Chọn một kỹ thuật trong ma trận để xem cảnh báo tương quan, máy chủ bị ảnh hưởng và dòng thời gian."}
              </SheetDescription>
            </div>
            {technique ? (
              <span className={`soc-badge ${maucDoViPham(detail?.maxSeverity ?? technique.maxSeverity)}`}>
                {muccDoViPham(detail?.maxSeverity ?? technique.maxSeverity)}
              </span>
            ) : null}
          </div>
        </SheetHeader>
          ...
          <div className="space-y-6 px-6 py-6">
            {loading && technique ? (
              ...
              <>
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                  <Metric
                    icon={Activity}
                    label="Tổng cảnh báo"
                    value={detail.totalAlerts.toLocaleString("vi-VN")}
                    tone="text-danger"
                  />
                  <Metric
                    icon={Shield}
                    label="Chiến thuật"
                    value={tacticToVi(technique.tacticLabel)}
                    tone="text-primary"
                  />
                  <Metric
                    icon={BadgeAlert}
                  ...
                  <Metric
                    icon={Server}
                    label="Máy chủ bị ảnh hưởng"
                    value={detail.affectedHostCount.toLocaleString("vi-VN")}
                    tone="text-info"
                  />
                </div>
                ...
                <div className="rounded-2xl border border-border/70 bg-secondary/10 p-4">
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <div>
                      <h3 className="text-sm font-semibold text-foreground">Dòng thời gian cảnh báo</h3>
                      <p className="mt-1 text-xs text-muted-foreground">Phân tích theo khoảng 30 phút trong 24 giờ qua</p>
                    </div>
                    {technique.url ? (
                      <a
                        href={technique.url}
                      ...
                      <AreaChart data={detail.timeline}>
                        ...
                        <RechartsTooltip
                          ...
                          formatter={(value: number, _name, item) => [
                            `${value.toLocaleString("vi-VN")} cảnh báo`,
                            `Mức độ cao nhất: ${muccDoViPham(Number(item.payload.maxSeverity ?? 0))}`,
                          ]}
                        />
                        <Area
                          type="monotone"
                    ...
                    <div className="space-y-2">
                      {detail.affectedHosts.map((host) => (
                        <div key={host.label} className="flex items-center justify-between rounded-xl border border-border/70 bg-background/30 px-3 py-2.5">
                          ...
                          <span className="font-mono text-xs text-info">{host.count.toLocaleString("vi-VN")}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                      ...
                      {detail.recentAlerts.map((alert) => (
                        <div key={alert.id} className="rounded-xl border border-border/70 bg-background/30 p-3">
                          <div className="mb-2 flex items-start justify-between gap-3">
                            ...
                            <span className={`soc-badge ${maucDoViPham(alert.level)}`}>
                              {muccDoViPham(alert.level)}
                            </span>
                          </div>
                          <div className="grid gap-2 text-xs text-muted-foreground md:grid-cols-2">
                            <div>
                              <span className="font-mono uppercase tracking-[0.18em]">IP nguồn</span>
                              <div className="mt-1 font-mono text-foreground">{alert.srcIp}</div>
                            </div>
                            <div>
                              <span className="font-mono uppercase tracking-[0.18em]">Máy chủ</span>
              ...
              <div className="rounded-2xl border border-dashed border-border/70 bg-secondary/10 px-4 py-10 text-center text-sm text-muted-foreground">
                Chọn một kỹ thuật trong ma trận để xem dòng thời gian chi tiết, máy chủ bị ảnh hưởng và cảnh báo gần đây.
              </div>
            )}
          </div>
        </ScrollArea>
